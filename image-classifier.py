...
def transform_image(img_path):
  ...
  try:
    ...
    print(f'Failed to open {img_path}. Corrupted file.')
...
def is_blank_image(image_path):
  ...
  if img is None:
    print(f"Image unable to load for {image_path}")
    return False
...
def hsv_classify_image(image_path, vg_threshold=0.95, real_threshold=0.07):
    """
...
#path = "/content/gdrive/MyDrive/experiments/assets"
#path2 = "/content/gdrive/MyDrive/experiments/model_logs"
...
model = GameNetV1()
if torch.cuda.is_available():
    model.load_state_dict(torch.load(path + "/mobile-net-v3-GAMENET700K-epochs-3-BS-512-LR-1e-3.pt", map_location=torch.device('cuda:0')))
else:
   model.load_state_dict(torch.load(path + "/mobile-net-v3-GAMENET700K-epochs-3-BS-512-LR-1e-3.pt", map_location=torch.device('cpu')))
def final_prediction(image_path, return_proba=False):

  # transform image
  image = transform_image(image_path)

  blank_img = is_blank_image(image_path)

  # check if it is a blank
  if blank_img:
    return 1

  hsv_result = hsv_classify_image(image_path)
...
screenshot_img_path = path + "/ScreenshotTest.png"
print(final_prediction(screenshot_img_path))